...
public class AdminController {
    ...
    @PostMapping("/pizzas")
    public ResponseEntity<?> createPizza(@RequestBody Pizza pizza, Authentication auth) {
    ...
    public ResponseEntity<?> updatePizza(@PathVariable Long id, @RequestBody Pizza updated, Authentication auth) {
        User u = userRepository.findByEmail(auth.getName()).orElse(null);
        if (u == null || !"ADMIN".equals(u.getRole())) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(new ErrorResponse("Access denied"));
        ...
        try {
            pizzaService.updatePizza(id, updated);
